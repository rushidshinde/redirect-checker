            ...
            <Card key={index} className={`${result.status === RedirectStatus.SUCCESS ? 'bg-emerald-100 text-emerald-600 dark:bg-emerald-100 dark:text-emerald-600' : result.status === RedirectStatus.WARNING ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-100 dark:text-yellow-800' : 'bg-red-100 text-red-600 dark:bg-red-100 dark:text-red-600'}`}>
              <CardHeader>
                <div className="flex justify-end items-center gap-4">
                  <div className={`rounded-full text-xs px-3 py-1 font-semibold border ${result.status_code === 200 ? 'border-emerald-600 text-emerald-600' : result.status_code === 404 ? 'border-red-600 text-red-600' : 'border-yellow-800 text-yellow-800'}`}>
              ...
              <CardContent>
                <div className="flex flex-col gap-1">
                  <p className="">
                    <span className="font-semibold">Source URL:</span> {result.source_url}
                  </p>
                  <p className="">
                    <span className="font-semibold">Target URL:</span> {result.target_url}
                  </p>
                  <p className="">
                    <span className="font-semibold">Redirected URL:</span> {result.redirected_url}
                  </p>
                </div>
              </CardContent>
              <CardFooter>
              ...
              {results?.map((result, index) => (
                <TableRow key={index} className={`${result.status === RedirectStatus.SUCCESS ? 'hover:bg-emerald-200 dark:hover:bg-emerald-200 hover:text-emerald-600 dark:hover:text-emerald-600' : result.status === RedirectStatus.FAILURE ? 'hover:bg-red-200 dark:hover:bg-red-200 hover:text-red-600 dark:hover:text-red-600' : 'hover:bg-yellow-200 dark:hover:bg-yellow-200 hover:text-yellow-800 dark:hover:text-yellow-800'}`}>
                  ...
                  <TableCell className="text-center">
                    <Badge
                      variant="outline"
                      className={`${result.status_code === 200 ? 'border-emerald-600 text-emerald-600 dark:border-emerald-600 dark:text-emerald-600' : result.status_code === 404 ? 'border-red-600 text-red-600 dark:border-red-600 dark:text-red-600' : 'border-yellow-800 text-yellow-800 dark:border-yellow-800 dark:text-yellow-800'}`}
                    >
                      {result.status_code}
                    </Badge>
                  </TableCell>
                  <TableCell className="">{result.source_url}</TableCell>
                  <TableCell className="">{result.target_url}</TableCell>
                  <TableCell className="">{result.redirected_url}</TableCell>
                  <TableCell className="">{result.message}</TableCell>
                  <TableCell className="text-center">{result.needs_update ? <>Yes</> : <>No</>}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
